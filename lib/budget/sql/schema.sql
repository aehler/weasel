...
create table weasel_main.budget_plan(
    id               bigserial primary key,
    user_id          bigint not null DEFAULT 0,
    organization_id  bigint not null DEFAULT 0,
    sum              NUMERIC not null DEFAULT 0,
...
create index idx_p_organization_id on weasel_main.budget_plan(organization_id);
create index idx_p_date on  weasel_main.budget_plan(date_op);
create index idx_p_tags on weasel_main.budget_plan using GIN(tags);